...
function baseCreateRenderer(
  ...
  const mountElement = (
    ...
    if (__DEV__ || __FEATURE_PROD_DEVTOOLS__) {
      ...
      // 只有根节点需要
      if (
        parentComponent!.subTree === vnode ||
        (parentComponent!.subTree &&
          parentComponent!.subTree.children &&
          (parentComponent!.subTree.children as any).includes(vnode))
      ) {
  ...
  const patchElement = (
    ...
    if (n2.props && (vnodeHook = n2.props.onVnodeBeforeUpdate)) {
