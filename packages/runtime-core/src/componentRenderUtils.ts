...
export function renderComponentRoot(
  ...
  try {
    if (vnode.shapeFlag & ShapeFlags.STATEFUL_COMPONENT) {
      ...
      const proxyToUse =
        (instance.type as any)['reactiveSetupState'] === false
          ? setupState
          : withProxy || proxy
    ...
    if (patchFlag & PatchFlags.FULL_PROPS) {
      ...
      for (let i = 0; i < dynamicProps.length; i++) {
        ...
        if (
          ...
          if (
            key === 'style' &&
            !hasStyleChanged(<any>nextProps![key], <any>prevProps![key])
          ) {
            continue
          }
          return true
...
function hasStyleChanged(nextVal: any, prevVal: any) {
  if (nextVal && prevVal) {
    ...
    if (nextValKeys.length != prevValKeys.length) return true
    for (let i = 0; i < nextValKeys.length; i++) {
      let key = nextValKeys[i]
      if (nextVal[key] !== prevVal[key]) return true
    }
  }
  return false
}
